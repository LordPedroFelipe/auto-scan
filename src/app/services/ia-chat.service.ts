...
export class IaChatService {
  ...
  gerarMensagemVendaComDados(data: any): string {
    if (!data) return '🚗 Não foi possível carregar os dados do veículo.';
    ...
    const precoFormatado = Number(price).toLocaleString('pt-BR', { style: 'currency', currency: 'BRL' });
    const kmFormatado = Number(mileage).toLocaleString('pt-BR');

    const caracteristicasExtras = [
      transmission ? `Transmissão: ${transmission}` : '',
      fuelType ? `Combustível: ${fuelType}` : '',
      condition ? `Condição: ${condition}` : '',
    ];

    if (optionals?.length) {
      const primeirosOpcionais = optionals.slice(0, 5).map((o: any) => o.description);
      caracteristicasExtras.push(...primeirosOpcionais);
    }

    return `🚗 Este é um ${brand} ${model} ${year}, motor ${engine || '---'}, com ${kmFormatado} km rodados.
      💰 Valor: ${precoFormatado}, com possibilidade de financiamento em até 60x!

      Características principais:
      👉 ${caracteristicasExtras.filter(Boolean).join('\n👉 ')}
